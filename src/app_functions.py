...
import pythoncom
import tempfile
# import winshell  # Removed due to PyInstaller compatibility issues
import wmi
try:
...
class Settings:
    def __init__(self):
        self.settings = QSettings("MK Apps", "CRTY PUBG Mobile Tool")
    ...
    def kill_adb():
        ...
        try:
            # ADB server'ı durdur
            subprocess.run(["adb", "kill-server"], timeout=10, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            # ADB process'lerini zorla kapat
            subprocess.run(["taskkill", "/F", "/IM", "adb.exe"], timeout=10, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            return True
...
class Optimizer(Registry):
    ...
    def change_dns_servers(dns_servers):
        ...
        try:
            ...
            # Return True if at least one adapter was successfully configured
            return success_count > 0 and total_adapters > 0
    ...
    def ipad_layout_settings(self, reset=False):
        ...
        backup_file = os.path.join(keymap_folder, 'TVM_100.xml.crtybackup')
        ...
        try:
            ...
            if _width is None or _height is None:
                try:
                    ...
                    if vm_res_width and vm_res_height:
                    ...
                    self.settings.setValue("VMResWidth", 1280)
                    self.settings.setValue("VMResHeight", 720)

            # Apply iPad layout settings
            try:
                self.ipad_layout_settings()
            except Exception as e:
                self.logger.error(f"iPad layout settings failed: {str(e)}")

            # Set new resolution
            if not self.set_dword("VMResWidth", width):
                raise Exception("Failed to set VMResWidth")
            if not self.set_dword("VMResHeight", height):
                raise Exception("Failed to set VMResHeight")
                
        except Exception as e:
            self.logger.error(f"iPad settings failed: {str(e)}")
            raise
...
class Game(Optimizer):
    ...
    def gen_game_icon(self, game_name):
        ...
        # Alternative to winshell.desktop() - get desktop path directly
        ...
        shortcut.Description = "By Mohamed Kamal (CRTY) - Discord: crty"
    ...
    def is_gameloop_running():
        ...
        try:
            running_process_list = subprocess.check_output(["tasklist"], timeout=10)
            emulator_processes = [b"AndroidEmulatorEx.exe", b"AndroidEmulatorEn.exe", b"AndroidEmulator.exe", b"aow_exe.exe"]
    ...
    def check_adb_connection(self, first_check=True):
        try:
            ...
            # İlk cihazı kullan (genellikle emulator-5554)
            self.adb = devices[0]
            
            # Cihazın hazır olup olmadığını kontrol et
            try:
                boot_complete = self.adb.shell("getprop sys.boot_completed", timeout=5)
                if boot_complete.strip() != "1":
                    raise Exception("Device not fully booted")
            except Exception:
                # Boot kontrolü başarısız olursa basit bir test yap
                self.adb.shell("echo test", timeout=3)
            
            # Test dosyası çek
            try:
                self.adb.sync.pull("/default.prop", self.resource_path(r'assets\testADB.crty'))
            except Exception:
                # default.prop yoksa alternatif dosya dene
                self.adb.sync.pull("/system/build.prop", self.resource_path(r'assets\testADB.crty'))
            
            self.is_adb_working = True
            self.logger.info("ADB connection successful")

        except Exception as e:
            self.logger.error(f"ADB connection failed: {str(e)}")
            self.kill_adb()
    ...
    def get_graphics_file(self, package: str):
        ...
        local_file_path = self.resource_path('assets/old.crty')
    ...
    def save_graphics_file(self):
        """Grafik dosyasını kaydet - Geliştirilmiş hata kontrolü ile"""
        try:
            file_path = self.resource_path("assets/new.crty")
            
            # Dosya yazma işlemini kontrol et
            ...
            if os.path.getsize(file_path) == 0:
                raise Exception("Dosya boş kaydedildi")
                
            return True
            
        except Exception as e:
            self.logger.error(f"Grafik dosyası kaydetme hatası: {str(e)}", exc_info=True)
            return False
    ...
    def set_fps(self, val: str) -> bool:
        ...
        try:
            fps_mapping = {
                "Düşük": b"\x02", "Low": b"\x02",
                "Orta": b"\x03", "Medium": b"\x03",
                "Yüksek": b"\x04", "High": b"\x04",
                "Ultra": b"\x05",
                "Extreme": b"\x06",
                "90 FPS": b"\x07", "Extreme+": b"\x07",
                "120 FPS": b"\x08", "Ultra Extreme": b"\x08"
            }
            fps_value = fps_mapping.get(val)

            if fps_value is None:
                self.logger.error(f"Geçersiz FPS değeri: {val}")
                return False
            ...
            for prop in fps_properties:
                try:
                    ...
                    if separator:  # Header bulundu
                        after = after[:1].replace(after[:1], fps_value) + after[1:]
                        self.active_sav_content = before + separator + after
                        success_count += 1
                    else:
                        self.logger.warning(f"FPS özelliği bulunamadı: {prop}")
                        
                except Exception as e:
                    self.logger.error(f"FPS ayarlama hatası ({prop}): {str(e)}")
            
            # En az bir özellik başarıyla ayarlandıysa başarılı say
            return success_count > 0
            
        except Exception as e:
            self.logger.error(f"FPS ayarlama genel hatası: {str(e)}", exc_info=True)
            return False
    ...
    def get_shadow(self):
        ...
        self.adb.sync.pull(user_custom_ini_path, self.resource_path(r'assets\user.crty'))

        with open(self.resource_path(r"assets\user.crty")) as file:
            for line in file:
                line = line.strip()
    ...
    def set_shadow(self, value):
        ...
        lines = []
        with open(self.resource_path(r"assets\user.crty"), "r") as file:
            for line in file:
        ...
        with open(self.resource_path(r"assets\user.crty"), "w") as file:
    ...
    def set_graphics_quality(self, quality):
        ...
        try:
            graphics_setting_dict = {
                "Pürüzsüz": b'\x01', "Smooth": b'\x01',
                "Dengeli": b'\x02', "Balanced": b'\x02',
                "HD": b'\x03',
                "HDR": b'\x04',
                "Ultra HD": b'\x05', "Ultra HD": b'\x05'
            }

            graphics_setting = graphics_setting_dict.get(quality)
            if graphics_setting is None:
                self.logger.error(f"Geçersiz grafik kalitesi: {quality}")
                return False
            ...
            for value in graphics_files:
                try:
                    if self.change_graphics_file(value, graphics_setting):
                        success_count += 1
                    else:
                        self.logger.warning(f"Grafik ayarı uygulanamadı: {value}")
                except Exception as e:
                    self.logger.error(f"Grafik ayarlama hatası ({value}): {str(e)}")
            
            # En az bir ayar başarıyla uygulandıysa başarılı say
            return success_count > 0
            
        except Exception as e:
            self.logger.error(f"Grafik kalitesi ayarlama genel hatası: {str(e)}", exc_info=True)
            return False
    ...
    def push_active_shadow_file(self):
        ...
        files = [
            (self.resource_path(r"assets\new.crty"), f"{data_dir}/SaveGames/Active.sav"),
            (self.resource_path(r"assets\user.crty"), f"{data_dir}/Config/Android/UserCustom.ini")
        ]
